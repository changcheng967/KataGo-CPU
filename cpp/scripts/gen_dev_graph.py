...
nodes.append(helper.make_node("Conv", ["InputSpatial", "stem_w"], ["stem_out"], pads=[1,1,1,1]))

# global: broadcast-add of Linear(19 -> C) per position
param("glob_w", (C, 19), 0.1)
nodes.append(helper.make_node("MatMul", ["InputGlobalFlat", "glob_w"], ["glob_proj"]))
nodes.append(helper.make_node("Add", ["stem_out", "glob_proj", "glob_bias_bcast"], ["stem2"]))
inits.append(t("glob_bias_bcast", np.zeros((1, 1, C), np.float32)))
...
for b in range(B):
    ...
    cos = np.tile(np.cos(ang), (1, 2)).transpose(1, 0).reshape(1, H, L, DH).astype(np.float32)
    sin = np.tile(np.sin(ang), (1, 2)).transpose(1, 0).reshape(1, H, L, DH).astype(np.float32)
    inits.append(t(f"{p}_cos", cos))
    inits.append(t(f"{p}_sin", sin))
    for nm in "qk":
        nodes.append(helper.make_node("Reshape", [f"{p}_{nm}", f"shape4"], [f"{p}_{nm}r"]))
        nodes.append(helper.make_node("Gather", [f"{p}_{nm}r", f"{p}_swap"], [f"{p}_{nm}s"], axis=3))
        nodes.append(helper.make_node("Mul", [f"{p}_{nm}r", f"{p}_cos"], [f"{p}_{nm}t1"]))
        nodes.append(helper.make_node("Mul", [f"{p}_{nm}s", f"{p}_sin"], [f"{p}_{nm}t2"]))
        nodes.append(helper.make_node("Add", [f"{p}_{nm}t1", f"{p}_{nm}t2"], [f"{p}_{nm}rope"]))
    ...
    nodes.append(helper.make_node("Reshape", [f"{p}_v", f"shape4"], [f"{p}_vr"]))
    nodes.append(helper.make_node("MatMul", [f"{p}_probs", f"{p}_vr"], [f"{p}_sv"]))
...
nodes.append(helper.make_node("Conv", ["head_na", "pol_w"], ["OutputPolicy"], pads=[1,1,1,1]))
nodes.append(helper.make_node("ReduceMean", ["head_na", "axes_pos"], ["pool"], keepdims=0))
inits.append(t("axes_pos", np.array([1], np.int64)))
param("val_w", (3, C), 0.1)
...
graph = helper.make_graph(
    ...
    [helper.make_tensor_value_info("InputSpatial", TensorProto.FLOAT, ["N", 22, 19, 19]),
     helper.make_tensor_value_info("InputGlobalFlat", TensorProto.FLOAT, ["N", 19]),
